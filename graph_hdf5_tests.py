# -*- coding: utf-8 -*-
"""
Created on Thu May 12 09:54:51 2022
...
#atest = coord2pixelOffset(aa, xys.X, xys.Y)
#sources = (np.array(atest[0])+1)*(np.array(atest[1])+1)-1
...
#f = h5py.File('cost_distances.h5', mode='a', rdcc_nbytes=1024**2*4000)
# (10,16671)
f = h5py.File('cost_distances.h5', mode='a')
...
# Write sparse matrices in chunks to an array in the same dataset
# Takes about xxs per chunk (291 chunks total when running 1000 nodes at a time)
...
for i in chunker(list(range(len(list(G8.nodes)))),cLen):
    ...
    darr = np.rint(darr/10).astype('uint16')
    #print(darr)
    if not exists('cost_distances_sp.h5'):
        f = h5sparse.File('cost_distances_sp.h5', mode='a')
        # Initialize with 
        f.create_dataset("dmat", maxshape=(None,), data=ss.csr_matrix(darr),
                                compression="gzip", shuffle=True,
                                chunks=True, dtype='uint16', compression_opts=6)
        f.close()
    else:
        with h5sparse.File('cost_distances_sp.h5', 'a', ) as f:
            f['dmat'].append(ss.csr_matrix(darr))
    toc = time.perf_counter()
    print(f"The subprocess took {toc - tic:0.4f} seconds")
...
tic = time.perf_counter()
elist = []
with h5sparse.File('cost_distances_sp.h5', 'r') as f:
    for p in [0,5000,8000,10000,20000,30000,50000,100000,150000,200000]:
        elist.append(f['dmat'][p:p+1])
toc = time.perf_counter()
print(f"Accessing rows took {toc - tic:0.4f} seconds")
...
with h5sparse.File('cost_distances_sp.h5', 'r') as f:
    for p in [(0,20),(5000,6000),(8000,200000),(10000,19000),
              (20000,40000),(30000,60000),(50000,120000),(100000,190000),
              (150000,200050),(200000,200060)]:
        elist.append(f['dmat'][p[1]:p[1]+1].toarray()[0,p[0]])
toc = time.perf_counter()
print(f"Accessing rows took {toc - tic:0.4f} seconds")
...
f = h5py.File('cost_distances.h5', mode='a')
#dset = f.create_dataset("dmat", (len(G8.nodes), len(G8.nodes)), compression="gzip", shuffle=True, chunks=True, dtype='f')
f.close()

xx = list(chunker(list(range(len(list(G8.nodes)))),cLen))[150:152]
for i in xx:
#for i in chunker(list(range(len(list(G8.nodes)))),cLen):
    tic = time.perf_counter()
    spsp = nk.distance.SPSP(aa, i)
    ...
    dists = spsp.getDistances()
    with h5py.File('cost_distances.h5', 'a') as f:
        # j is relative to 0, k is relative to chunk indices
        for j,k in enumerate(i):
            dset = f.create_dataset(str(k), len(dists[j][k:len(dists[j])]), compression="gzip", shuffle=True, chunks=True, dtype='f')
            dset[:] = dists[j][k:len(dists[j])]
    toc = time.perf_counter()
    print(f"The subprocess took {toc - tic:0.4f} seconds")
